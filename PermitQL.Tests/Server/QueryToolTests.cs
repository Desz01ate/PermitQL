...
public sealed class QueryToolTests
{
    ...
    {
        this._pipeline.ExecuteAsync("SELECT tags FROM t", "k", Arg.Any<CancellationToken>())
    ...
    {
        this._pipeline.ExecuteAsync("SELECT ids FROM t", "k", Arg.Any<CancellationToken>())
            .Returns(new QueryResult(
                [new(0, "ids", "int4[]", true)],
    ...
    {
        this._pipeline.ExecuteAsync("SELECT name FROM t", "k", Arg.Any<CancellationToken>())
            .Returns(new QueryResult(
    ...
    public async Task Markdown_ByteArrayCell_FormatsAsBase64()
    {
        ...
        this._pipeline.ExecuteAsync("SELECT data FROM t", "k", Arg.Any<CancellationToken>())
            .Returns(new QueryResult(
    ...
    public async Task Markdown_BitArrayCell_FormatsAsBitString()
    {
        ...
        this._pipeline.ExecuteAsync("SELECT flags FROM t", "k", Arg.Any<CancellationToken>())
            .Returns(new QueryResult(
    ...
    public async Task Markdown_DictionaryCell_FormatsAsJson()
    {
        ...
        this._pipeline.ExecuteAsync("SELECT meta FROM t", "k", Arg.Any<CancellationToken>())
            .Returns(new QueryResult(
    ...
    public async Task Markdown_GuidCell_FormatsWithoutQuotes()
    {
        ...
        this._pipeline.ExecuteAsync("SELECT id FROM t", "k", Arg.Any<CancellationToken>())
            .Returns(new QueryResult(
                [new(0, "id", "uuid", false)],
                [new object?[] { guid }]));
    ...
    public async Task Markdown_DateTimeOffsetCell_FormatsAsInvariantString()
    {
        ...
        this._pipeline.ExecuteAsync("SELECT ts FROM t", "k", Arg.Any<CancellationToken>())
            .Returns(new QueryResult(
